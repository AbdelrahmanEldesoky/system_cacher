                                                            ...
                                                            <tr>
                                                                ...
                                                                <td id="Features">
                                                                    @foreach($product->features as $feature)
                                                                        <div>
                                                                            <label for="feature_{{$feature->id}}"> {{ $feature->feature }} </label>
                                                                            <input type="checkbox" name="feature_{{$feature->id}}" id="feature_{{$feature->id}}" value="{{$feature->feature}}" onchange="Toggle_Features(this.checked,this.value,'{{ $product->id }}')">
                                                                        </div>
                                                                    @endforeach
                                                                </td>
                                                                {{-- <td>
                    ...
                    <div class="box box-primary">
                        ...
                        <div class="box-body">
                            ...
                            <form action="{{ route('dashboard.clients.orders.update', ['order' => $order->id, 'client' => $client->id]) }}" method="post">
                                ...
                                <table class="table table-hover">
                                    ...
                                    @foreach ($order->products as $product)
                                        <tr></tr>
                                            <td>{{ $product->name }}</td>
                                            <td>
                                                <input type="number" name="products[{{ $product->id }}][quantity]" data-price="{{ number_format($product->sale_price, 2) }}" class="form-control input-sm product-quantity" min="1" value="{{ $product->pivot->quantity }}">
                                            </td>
                                                <input type="hidden" name="products[{{ $product->id }}][features]"
                                                data-features="{{ $product->pivot->features }}"
                                                value="{{$product->pivot->features}}">

                                                <input type="hidden" name="products[{{ $product->id }}][is_finsh]"  
                                                value="{{$product->pivot->is_finsh}}">

                                                <td class="product-price">{{ number_format($product->sale_price * $product->pivot->quantity, 2) }}</td>

                                               <td> <input type="text" name="products[{{ $product->id }}][note]"  value="{{ $product->pivot->note }}">

                                               </td>
                                                    


                                                    <td class="product-features" >{{$product->pivot->features}}</td>
                                ...
                                <h4><span> خصــــــم :<input type="number" name="discount"   min="0" value="{{$order->discount}}">
                                </span></h4>
